...
export class CouponModel {
    ...
    convertObj(data: any) {
        ...
        this.id = data?.id ?? 0;
        this.shop = data?.shop ? new ShopModel().convertObj(data?.shop) : undefined;
        this.name = data?.name ?? "";
        this.code = data?.code ?? "";
        this.discount_type = data?.discount_type ?? DiscountTypes.PERCENTAGE;
        ...
        this.max_usage = data?.max_usage ?? -1;
        this.valid_from = data?.valid_from ? new Date(data.valid_from) : null;
        this.valid_to = data?.valid_to ? new Date(data.valid_to) : null;
        this.is_used = data?.is_used ?? false;
        this.is_saved = data?.is_saved ?? false;
